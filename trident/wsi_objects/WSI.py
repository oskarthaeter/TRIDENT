...
class OpenSlideWSI:
    ...
    def _fetch_magnification(
        self, custom_mpp_keys: List[str] | None = None
    ) -> int | None:
        """
        ...
        try:
            ...
            magnification = self.img.properties.get(
    ...
    def segment_tissue(
        ...
        width, height = int(round(width * mpp_reduction_factor)), int(
            round(height * mpp_reduction_factor)
        )
        ...
        if not holes_are_tissue:
            holes, _ = cv2.findContours(
                predicted_mask, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE
            )
            for hole in holes:
    ...
    def visualize_coords(self, coords_path: str, save_patch_viz: str) -> str:
        ...
        canvas = np.array(
            self.get_thumbnail((thumbnail_width, thumbnail_height))
        ).astype(np.uint8)
        ...
        cv2.putText(
            canvas,
            f"patch={patch_size} w. overlap={overlap} @ {target_magnification}x",
        ...
        viz_coords_path = os.path.join(
            save_patch_viz, f"{os.path.splitext(os.path.basename(self.name))[0]}.jpg"
        )
