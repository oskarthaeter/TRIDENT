...
class OpenSlideWSIPatcher:
    ...
    def __init__(
        ...
        if self.mask is not None:
            self.valid_patches_nb, self.valid_coords = self._compute_masked(
                coords, threshold
            )
        else:
    ...
    def _xy_to_colrow(self, x, y):
        ...
        if x == 0:
            ...
            col = (
                (x - self.patch_size_src) // (self.patch_size_src - self.overlap_src)
            ) + 1
        ...
        if y == 0:
            ...
            row = (
                (y - self.patch_size_src) // (self.patch_size_src - self.overlap_src)
            ) + 1
    ...
    def _compute_masked(self, coords, threshold, simplify_shape=True) -> None:
        ...
        if simplify_shape:
            mask = self.mask.simplify(
                tolerance=self.patch_size_target / 4, preserve_topology=True
            )
        else:
            mask = self.mask
    ...
    def _prepare(self) -> None:
        level, _ = self.wsi.get_best_level_and_custom_downsample(
            self.downsample, tolerance=0.1
        )
    ...
    def get_tile_xy(self, x: int, y: int) -> Tuple[np.ndarray, int, int]:
        if self.pil:
            ...
            if self.patch_size_target is not None:
                tile = cv2.resize(
                    tile, (self.patch_size_target, self.patch_size_target)
                )[:, :, :3]
