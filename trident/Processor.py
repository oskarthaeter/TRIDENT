...
class Processor:
    def __init__(
        ...
        for ext in self.wsi_ext:
            assert ext.startswith(
                "."
            ), "Each extension in wsi_ext must start with a period."
            valid_slides.extend(
                [name for name in os.listdir(wsi_source) if name.endswith(ext)]
            )
        ...
        if custom_list_of_wsis is not None:
            ...
            if "wsi" not in wsi_df.columns:
                raise ValueError(
                    "CSV with custom list of WSIs must contain a column named 'wsi'."
                )
        ...
        if self.wsi_cache:
            ...
            print(
                f"Using local cache at {wsi_cache}, which currently contains {len(os.listdir(wsi_cache))} files."
            )
        ...
        for wsi_idx, wsi in enumerate(valid_slides):
            wsi_path = (
                os.path.join(self.wsi_cache, wsi)
                if self.wsi_cache is not None
                else os.path.join(self.wsi_source, wsi)
            )

            # Get path to segmentation
            tissue_seg_path = os.path.join(
                self.job_dir, "contours_geojson", f"{os.path.splitext(wsi)[0]}.geojson"
            )
    ...
    def populate_cache(self) -> None:
        ...
        for wsi in self.loop:
            ...
            ) and not is_locked(os.path.join(self.wsi_cache, f"{wsi.name}{wsi.ext}")):
                self.loop.set_postfix_str(
                    f"{wsi.name}{wsi.ext} already in cache. Skipping..."
                )
                update_log(
                    os.path.join(self.wsi_cache, "_logs.txt"),
                    f"{wsi.name}{wsi.ext}",
                    "In cache",
                )
                continue
            ...
            if is_locked(os.path.join(self.wsi_cache, f"{wsi.name}{wsi.ext}")):
                ...
                update_log(
                    os.path.join(self.wsi_cache, "_logs.txt"),
                    f"{wsi.name}{wsi.ext}",
                    "LOCKED. Moving to cache...",
                )
                shutil.copy(
                    os.path.join(self.wsi_source, f"{wsi.name}{wsi.ext}"),
                    self.wsi_cache,
                )
        ...
        for wsi in self.loop:
            ...
            if self.wsi_cache is not None:
                if is_locked(
                    ...
                    self.loop.set_postfix_str(
                        f"{wsi.name} not found in cache. Skipping..."
                    )
                    update_log(
                        os.path.join(self.job_dir, "_logs_segmentation.txt"),
                        f"{wsi.name}{wsi.ext}",
                        "WSI not found in cache.",
                    )
                    continue
    ...
    def run_patching_job(
        ...
        export_entropy_format: bool = True,
    ) -> str:
        """
        ...
        for wsi in self.loop:
            ...
            try:
                ...
                wsi.extract_tissue_coords(
                    ...
                    export_entropy_format=export_entropy_format,
                )
    ...
    def run_slide_feature_extraction_job(
        ...
        if slide_encoder.enc_name.startswith("mean-"):
            slide_to_patch_encoder_name[
                slide_encoder.enc_name
            ] = slide_encoder.enc_name.split("mean-")[
                1
        ...
        if len(already_processed) < len(self.wsis):
            print(
                f"Some patch features haven't been extracted in {len(already_processed)}/{len(self.wsis)} WSIs. Starting extraction."
            )
    ...
    def cleanup(self, filename: str) -> None:
        ...
        """
        if (
            self.wsi_cache is not None
            and self.clear_cache
            and os.path.exists(os.path.join(self.wsi_cache, filename))
        ):
