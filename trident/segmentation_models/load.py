...
class HESTSegmenter(SegmentationModel):
    ...
    def _build(self, checkpoint_dir, device):
        ...
        model.classifier[4] = nn.Conv2d(
            in_channels=256, out_channels=2, kernel_size=1, stride=1
        )
        ...
        if not os.path.isfile(os.path.join(checkpoint_dir, "deeplabv3_seg_v4.ckpt")):
            try:
                ...
                raise Exception(
                    "Please install huggingface_hub (`pip install huggingface_hub`) to use this model"
                )
        ...
        checkpoint = torch.load(
            os.path.join(checkpoint_dir, "deeplabv3_seg_v4.ckpt"),
...
class GrandQCSegmenter(SegmentationModel):
    ...
    def _build(self, checkpoint_dir, device):
        ...
        model.load_state_dict(
            torch.load(os.path.join(checkpoint_dir, MODEL_TD_NAME), map_location="cpu")
        )
        model.to(device)
        model.eval()
...
def segmentation_model_factory(
    model_name, confidence_thresh=0.5, device="cuda", freeze=True
):
    """
    ...
    if model_name == "hest":
        return HESTSegmenter(
            ...
            checkpoint_dir=os.path.join(current_dir, "hest-tissue-seg/"),
            device=device,
        )
        ...
        return GrandQCSegmenter(
            freeze,
            confidence_thresh=confidence_thresh,
            checkpoint_dir=os.path.join(current_dir, "grandqc/"),
            device=device,
        )
    else:
